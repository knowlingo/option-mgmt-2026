...
**Phase 1 — Engine MVP. All milestones (M1.x) shipped; one infra follow-up remains — Playwright E2E CI activation (see M1.25).** Engine `1.8.0`. The decision pipeline is feature-complete end-to-end through M1.13, the **Master Decision Engine auto-dispatches `OPEN_COLLAR` emits** to the Collar Builder (M1.11b — `produce_daily_decision()` attaches a `CollarStructure` to `DailyDecision.collar_structures` when the rule pipeline emits `OPEN_COLLAR`), the **public API surface is shipped** (M1.14–M1.17.5 + M1.16a: 18 endpoints), the **Today screen scaffolding is live** (M1.18), the **Collar Builder is engine + dispatch + API complete** (M1.11a engine + M1.11b dispatch + M1.16a endpoint), and the **M1.24 golden `DailyDecision` suite is locked** (12 regenerated snapshots + `engine.decision.serialize_canonical` + CHANGELOG-drift CI guard, shipped at engine `1.7.0`). The engine is at **`1.8.0`** because the parallel **yearline** track's OM-Y1 `YearlineContext` contract bumped it past M1.24 (see the Yearline integration section below). The Today screen now renders the **complete Phase-1 DailyDecisionCard**: live actions + per-leg execution feasibility (M1.19), the confidence breakdown + aggregate execution panel (M1.20), and watch levels + Why/Risks/Invalidation drawers (M1.21) — no placeholder slots remain. The **Settings screen is now live** (M1.22): a dependency-free `UserStrategyProfileForm` over the 8-field `UserStrategyProfile` + Helen/Ravi/Diana persona presets, saved through `PUT /profile` via a server action. The **Outcomes screen is now live** (M1.23): manual outcome entry + cursor-paginated history + inline edit + summary stats over `/outcomes`, closing the decision→outcome loop (`pnl_*` are `Decimal`→JSON strings; mutations via server actions). All three Phase-1 surfaces (Today · Settings · Outcomes) ship. **M1.25** lands the confidence-calibration scaffold (`tests/test_calibration.py`) + authored-but-**dormant** Playwright E2E (excluded from CI; `@playwright/test` can't be added under `--frozen-lockfile` here) + polish — so every Phase-1 milestone (M1.x) is now shipped. The one open item is activating the Playwright E2E in CI (a lockfile regen + an `e2e` job; the real calibration corpus is M3.10). See the [M1.25 dev spec](./docs/phased-design/phase-1/m1.25-calibration-e2e-polish.md).
...
| ✅ | M1.19 — Today actions: `ActionList` + `ActionRow` + `ExecutionBadge` (per-action execution feasibility; `OPEN_COLLAR` renders both collar legs; first consumer of `DailyDecision.collar_structures`) | [#12](https://github.com/knowlingo/option-mgmt-2026/pull/12) | — |
| ✅ | M1.20 — `ConfidenceBreakdownChart` (Recharts `StackedConfidenceBar` + 6-segment breakdown + §22.13 arithmetic caption) + `ExecutionFeasibilityPanel`; added the `--chart-1..5` CSS tokens | [#13](https://github.com/knowlingo/option-mgmt-2026/pull/13) | — |
| ✅ | M1.21 — `WatchLevels` + Why/Risks/Invalidation drawers (native `<details>`; completes the DailyDecisionCard tree — no `PlaceholderCard` remain; `WatchLevels` is a forward-typed seam pending an engine `watch_levels` follow-up) | [#15](https://github.com/knowlingo/option-mgmt-2026/pull/15) | — |
| ✅ | M1.22 — Settings page: `UserStrategyProfileForm` (8-field `UserStrategyProfile`) + `PersonaPresetButtons` (Helen/Ravi/Diana) + Today·Settings nav; dependency-free (native controls + `saveProfile` server action; no RHF/zod/react-query/shadcn) | [#17](https://github.com/knowlingo/option-mgmt-2026/pull/17) | — |
| ✅ | M1.23 — Outcomes screen: manual entry + cursor-paginated history + inline edit + stats over `/outcomes`; dependency-free (server actions; `pnl_*` are `Decimal`→JSON strings); authored the dev spec (none existed) | [#19](https://github.com/knowlingo/option-mgmt-2026/pull/19) | — |
| ✅ | M1.25 — Calibration scaffold (`tests/test_calibration.py`: bin-by-confidence monotonicity + golden integration smoke) + **dormant** Playwright E2E (`apps/web/e2e/`, excluded from CI; activation documented) + per-route titles; authored the dev spec (none existed) | [#21](https://github.com/knowlingo/option-mgmt-2026/pull/21) | — |
